...
def transform_freebase_to_internal_format(
    data_path_in: str, data_path_out: str
) -> None:
...
def _transform_spo_to_internal_format(data_path_in: str, data_path_out: str) -> None:
    with open(data_path_in, "r", encoding="utf-8") as file_in, open(
        data_path_out, "w", encoding="utf-8"
    ) as file_out:
