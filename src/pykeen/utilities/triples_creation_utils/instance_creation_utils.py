...
def create_mapped_triples(
    ...
    triples_of_ids = np.concatenate(
        [subject_column, relation_column, object_column], axis=1
    )

    triples_of_ids = np.array(triples_of_ids, dtype=np.long)
    # Note: Unique changes the order
    return (
        np.unique(ar=triples_of_ids, axis=0),
        entity_label_to_id,
...
def create_mappings(triples: np.ndarray) -> Tuple[Dict[str, int], Dict[str, int]]:
    ...
    entities = np.unique(
        np.ndarray.flatten(np.concatenate([triples[:, 0:1], triples[:, 2:3]]))
    )
    entity_label_to_id = {
        entity_label: entity_id for entity_id, entity_label in enumerate(entities)
    }
