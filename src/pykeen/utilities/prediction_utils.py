...
__all__ = ["create_triples", "make_predictions"]


def create_triples(entity_pairs, relation) -> np.ndarray:
    subjects = entity_pairs[:, 0:1]
    objects = entity_pairs[:, 1:2]
    relation_repeated = np.reshape(
        np.repeat(relation, repeats=subjects.shape[0]), newshape=(-1, 1)
    )
    return np.concatenate([subjects, relation_repeated, objects], axis=1)


def make_predictions(
    kge_model, entities, relations, entity_to_id, rel_to_id, device, blacklist_path=None
):
    ...
    if relations.size == 1:
        ...
        all_triples = create_triples(
            entity_pairs=all_entity_pairs, relation=relations[0]
        )
    ...
    mapped_triples, _, _ = create_mapped_triples(
        all_triples, entity_label_to_id=entity_to_id, relation_label_to_id=rel_to_id
    )
