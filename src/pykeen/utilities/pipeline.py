...
class Pipeline(object):
    ...
    def __init__(self, config: Dict):
        ...
        self.device_name = (
            ...
            and self.config[pkc.PREFERRED_DEVICE] == pkc.GPU
            else pkc.CPU
        )
    ...
    def run(self) -> Mapping:
        ...
        # Prepare Output
        entity_id_to_label = {
            value: key for key, value in self.entity_label_to_id.items()
        }
        relation_id_to_label = {
            value: key for key, value in self.relation_label_to_id.items()
        }
        entity_label_to_embedding = {
            entity_id_to_label[entity_id]: embedding.detach().cpu().numpy()
            for entity_id, embedding in enumerate(
                trained_model.entity_embeddings.weight
            )
        }
        ...
        if self.config[pkc.KG_EMBEDDING_MODEL_NAME] in (pkc.SE_NAME, pkc.UM_NAME):
            ...
            relation_label_to_embedding = {
                ...
                for relation_id, embedding in enumerate(
                    trained_model.relation_embeddings.weight
                )
            }
