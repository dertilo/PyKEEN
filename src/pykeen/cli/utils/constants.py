...
KG_MODEL_TO_ID_MAPPING = OrderedDict(
    {value: key for key, value in ID_TO_KG_MODEL_MAPPING.items()}
)

ID_TO_OPTIMIZER_MAPPING = OrderedDict(
    {"1": SGD_OPTIMIZER_NAME, "2": ADAGRAD_OPTIMIZER_NAME, "3": ADAM_OPTIMIZER_NAME}
)

OPTIMIZER_TO_ID_MAPPING = OrderedDict(
    {value: key for key, value in ID_TO_OPTIMIZER_MAPPING.items()}
)
