...
def export_experimental_artifacts(results: Mapping, output_directory: str) -> None:
    ...
    if eval_summary is not None:
        with open(
            os.path.join(output_directory, "evaluation_summary.json"), "w"
        ) as file:
...
def run(config: Dict, output_directory: Optional[str] = None) -> Results:
    """Train a KGE model.
    ...
    if output_directory is None:
        ...
        output_directory = os.path.join(
            config[OUTPUT_DIREC], time.strftime("%Y-%m-%d-%H-%M-%S")
        )
    ...
    export_experimental_artifacts(results=results, output_directory=output_directory)

    return Results(config=config, pipeline=pipeline, results=results)
